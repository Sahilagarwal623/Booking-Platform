...
export default function EventFilters({ currentFilters, onFilterChange, onClear }: EventFiltersProps) {
    ...
    return (
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 space-y-8 sticky top-24">
            ...
            <div>
                ...
                <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span className="text-slate-400 text-sm">All Categories</span>
                    </label>
                    {CATEGORIES.map((cat) => (
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <input
                        ...
                        className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-indigo-500"
                    />
                    <input
                        type="number"
                        placeholder="Max"
            ...
            {/* City */}
            <div>
                <h3 className="text-sm font-medium text-slate-300 mb-3">City</h3>
                <input
                    type="text"
                    placeholder="Enter city"
                    value={localFilters.city || ''}
                    onChange={(e) => handleChange('city', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-hidden focus:border-indigo-500"
                />
            </div>

            {/* Date Range */}
            <div>
                <h3 className="text-sm font-medium text-slate-300 mb-3">Date</h3>
                <div className="space-y-2">
                    <input
                        type="date"
